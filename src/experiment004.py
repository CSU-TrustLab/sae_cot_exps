...
# Experiment 004: same as 003 but wraps the prompt in the DeepSeek chat template
# (<｜User｜>…<｜Assistant｜>) to give the model an explicit role context. This produces
# a cleaner boundary between the question, the <think> trace, and the final answer.
# Reports top-5 active SAE features at six epochs:
# a) first question content token, b) last question token (?),
# c) first thinking token, d) last thinking token,
# e) first answer token, f) last answer token.
...
# Ġ (U+0120 — the BPE word-boundary marker) in the content bypasses this while still
# letting apply_chat_template inject the role tokens (<｜User｜>, <｜Assistant｜>) correctly
# as special tokens (they bypass the normalizer entirely).
_prompt_bpe = prompt.replace(" ", "Ġ")
messages = [{"role": "user", "content": _prompt_bpe}]
# Step 1: build the formatted string (BOS + role markers already included).
# Step 2: tokenize it — the tokenizer splits on special tokens first (so <｜User｜> etc.
# are looked up directly), then only the plain content runs through the normalizer,
# where Ġ characters survive because the normalizer only strips ASCII 0x20.
_formatted = hf_tokenizer.apply_chat_template(messages, tokenize=False, add_generation_prompt=True)
input_ids = hf_tokenizer(_formatted, add_special_tokens=False, return_tensors="pt").input_ids.to(device)
n_prompt = input_ids.shape[1]
print("DEBUG str_tokens(input)[:20]:", model.to_str_tokens(input_ids[0])[:20])

output_tokens = model.generate(
    input_ids,
    max_new_tokens=512,
    stop_at_eos=True,
    prepend_bos=False,  # BOS already in input_ids via chat template
    return_type="tokens",
)
...
# Skip BOS and DeepSeek role markers (<｜…｜>) to reach the first question content token.
first_prompt_pos = 1 if (output_tokens[0][0].item() == bos_id) else 0
while first_prompt_pos < n_prompt and str_tokens[first_prompt_pos].startswith("<｜"):
    first_prompt_pos += 1

# Last question token: '?' marks the end of the question.
# Search backwards from n_prompt-1 (the <｜Assistant｜> token) to find '?'.
...
epochs = [
    ("Epoch a) first question token",  first_prompt_pos),
