...
# Experiment 005: DeepSeek-R1-Distill-Llama-8B with two SAEs, each hooked independently
# to its own layer — one at layer 0, one at layer 31 (llama_scope_r1_distill /
# fnlp/Llama-Scope-R1-Distill, 32k features each). Compares early-layer (L0) vs
# late-layer (L31) feature activations at six epochs of generation:
# a) first prompt token, b) last question token (?), c) first thinking token,
...
SAE_CFGS = [
    {"sae_id": "l0r_800m_slimpajama",  "sae_name": "0-llamascope-slimpj-openr1-res-32k"},
    {"sae_id": "l31r_800m_slimpajama", "sae_name": "31-llamascope-slimpj-openr1-res-32k"},
]
...
loaded_saes = []
for cfg in SAE_CFGS:
    sae = SAE.from_pretrained(sae_release, cfg["sae_id"], device=device)
    loaded_saes.append(sae)
    print("+SAE {} loaded".format(cfg["sae_id"]))
...
_, cache = model.run_with_cache_with_saes(output_tokens, saes=loaded_saes)
sae_acts = {
    cfg["sae_id"]: cache[sae.cfg.metadata.hook_name + ".hook_sae_acts_post"][0]
    for cfg, sae in zip(SAE_CFGS, loaded_saes)
}  # sae_id -> tensor[pos, n_features]
...
def print_top_features(label, pos, decoded_tokens, k=5):
    print(f"\n{label}")
    print(f"  token [{pos}]: {repr(decoded_tokens[pos])}")
    for cfg, sae in zip(SAE_CFGS, loaded_saes):
        acts = sae_acts[cfg["sae_id"]]
        exps = explanations[cfg["sae_id"]]
        print(f"  [{cfg['sae_id']}]")
        vals, inds = torch.topk(acts[pos], k)
        for val, ind in zip(vals, inds):
            desc = exps.get(ind.item(), "no description")
            print(f"    F{ind.item():<6} {val:.2f}  {desc}")

for label, pos in epochs:
    print_top_features(label, pos, decoded_tokens)
